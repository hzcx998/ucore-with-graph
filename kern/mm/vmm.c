...
mm_create(void) {
    ...
    if (mm != NULL) {
        ...
        mm->brk = mm->brk_start = 0;  // brk start value
    }    
    return mm;
}
...
void 
mm_print_vmare(struct mm_struct *mm) {
    struct vma_struct *vma, *prev;
    list_for_each_owner (vma, &mm->mmap_list, list_link) {
        cprintf("vma: start: %x end:%x flags:%x\n", vma->vm_start, vma->vm_end, vma->vm_flags);
        prev = vma;
    }
    cprintf("final vma: start: %x end:%x flags:%x\n", prev->vm_start, prev->vm_end, prev->vm_flags);
    
}

// 保证值和页的大小对齐
#define PGALIGN(value) ((value + PGSIZE - 1) & (~(PGSIZE - 1)))

/**
 * mm_set_brk - set brk start value
 * 
 */
void 
mm_set_brk(struct mm_struct *mm) {
    struct vma_struct *vma, *prev;
    list_for_each_owner (vma, &mm->mmap_list, list_link) {
        //cprintf("vma: start: %x end:%x flags:%x\n", vma->vm_start, vma->vm_end, vma->vm_flags);
        prev = vma;
 ...
 * 把地址和长度范围内的空间纳入堆的管理。
 */
static unsigned int do_brk(struct mm_struct *mm, unsigned int addr, unsigned int len)
{
    /* 页对齐后检查长度，如果为0就返回 */
    len = PGALIGN(len);
    if (!len) 
    ...
    vma = vma_create(addr, addr + len, VM_READ | VM_WRITE);
    if (vma == NULL)
        return -1;
...
uintptr_t mm_brk(uintptr_t addr)
{
    unsigned int ret;
    unsigned int old_brk, new_brk;
    struct mm_struct *mm = current->mm;
    
    /*printk(KERN_DEBUG "sys_vmspace_addr: vmm addr start %x end %x new %x\n", 
        vmm->brk_start, vmm->brk, addr);*/

    /* 如果堆比开始位置都小就退出 */
    if (addr < mm->brk_start) {
        //printk(KERN_DEBUG "sys_vmspace_addr: new addr too low!\n");
        goto the_end;
    }
    
    /* 使断点值和页对齐 */
    new_brk = PGALIGN(addr);
    old_brk = PGALIGN(mm->brk);

    /* 如果新旧堆值在同一个页内，就设置新的堆值然后返回 */
    if (new_brk == old_brk) {
        //printk(KERN_DEBUG "sys_vmspace_addr: both in a page!\n");
        goto set_addr; 
    }
    
    /* 如果addr小于当前vmm的brk，就说明是收缩内存 */
    if (addr <= mm->brk && addr >= mm->brk_start) {
        //printk(KERN_DEBUG "sys_vmspace_addr: shrink mm.\n");
        
        /* 收缩地址就取消映射，如果成功就去设置新的断点值 */
        /*if (!do_vmspace_unmap(vmm, new_brk, old_brk - new_brk))
            goto set_addr;
        */    
        cprintf("sys_brk: unmap failed!\n");
        goto the_end;
    }
    
    /* 检查是否超过堆的空间限制 */
    if (addr > mm->brk_start + HEAP_SIZE)
        goto the_end;
    
    /* 堆新的断点进行空间映射 */
    if (do_brk(mm, old_brk, new_brk - old_brk) != old_brk) {
        cprintf("sys_brk: do_brk failed! addr %x len %x\n",
            old_brk, new_brk - old_brk);
        goto the_end;
    }
set_addr:
    /*printk(KERN_DEBUG "sys_vmspace_addr: set new addr %x old is %x\n",
        addr, vmm->brk);*/
