...
{
    KGC_Label_t *label = kmalloc(sizeof(KGC_Label_t));
    if (label == NULL)
        return NULL;
...
int KGC_LabelInit(KGC_Label_t *label)
{
    ...
    label->text = kmalloc(label->textMaxLength);
    if (label->text == NULL) {
        return -1;
    }
    /* 进行默认的初始化 */
    ...
    int height, uint8_t *raw, kgcc_t border, kgcc_t fill)
{
    ...
    if (raw == NULL) {
        ...
        label->image.data = kmalloc(width * height * KGC_CONTAINER_BPP);
        if (label->image.data) {
            /* 开始数据绘制 */
            /* 图形类型 */
 ...
 * 从容器上删除标签
 */
void KGC_DelLabel(KGC_Label_t *label)
{
