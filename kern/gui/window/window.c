...
/* 窗口链表头，所有窗口都在此链表上 */
LIST_HEAD(windowListHead);

/* 当前激活的窗口 */
KGC_Window_t *currentWindow;

/* 创建窗口皮肤 */
KGC_WindowSkin_t windowSkin;

/* 窗口移动 */
KGC_WindowMovement_t windowMovement;
    ...
    void *param)
{
    ...
    /* 创建窗口对象 */
    KGC_Window_t *window = (KGC_Window_t *) kmalloc(sizeof(KGC_Window_t));
    if (window == NULL)
        return NULL;
    ...
    /* 消息队列 */
    list_init(&window->messageListHead);
 ...
 * 
 */
int KGC_WindowAdd(KGC_Window_t *window)
{
    if (!window)
        return -1;
    ...
    /* 把窗口添加到窗口链表 */
    list_add_tail(&window->list, &windowListHead);
    /* 设置为当前窗口 */
 ...
 * 
 */
int KGC_WindowDel(KGC_Window_t *window)
{
    /* 从窗口队列中删除 */
    ...
    if (window->task) {
        /* 任务窗口为空 */
        struct proc_struct *proc = (struct proc_struct *)window->task;
        /* 解除任务与窗口的绑定 */
        proc->window = NULL;    
        /* 解除窗口与任务的绑定 */
        window->task = NULL;
 ...
 * 成功返回0，失败返回-1
 */
int KGC_WindowDestroy(KGC_Window_t *window)
{
    if (!window)
        return -1;
 ...
 * 关闭指定窗口
 */
int KGC_WindowClose(KGC_Window_t *window)
{
    if (!window)
        return -1;
    
    /* 如果是窗口的创建者，才能够关闭窗口 */
    if (window->task != current)
        return -1;
...
#if 0
static void ThreadWindow(void *arg)
{
...
int KGC_InitWindowContainer()
{
    ...
    /* 全屏大小 */
    KGC_ContainerInit(windowMovement.walker, "windowMovement", 0, 0, videoInfoPtr->xResolution, 12, NULL);

    /* 全透明 */
    KGC_ContainerDrawRectangle(windowMovement.walker, 0, 0, videoInfoPtr->xResolution, 12, KGCC_NONE);
    ...
    /*
    int pid = kernel_thread(ThreadWindow, NULL, 0);
    if (pid <= 0) {
        panic("create ThreadWindow failed.\n");
    }*/
